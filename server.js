...
const getProductos = (tienda) => {
    ...
    if (!fs.existsSync(filePath)) {
        // Crear archivo con array VACÍO (sin productos por defecto)
        const productosVacios = [];
        fs.writeFileSync(filePath, JSON.stringify(productosVacios, null, 2));
        console.log(`📁 Archivo de productos creado para tienda "${tienda}" - ESTÁ VACÍO`);
        return productosVacios;
    }
    
    // Leer y devolver los productos existentes
    try {
...
app.delete('/api/admin/productos/:id', AUTH, (req, res) => {
    ...
    // Limpiar categorías vacías después de eliminar productos
    limpiarCategoriasVacias(tienda);
