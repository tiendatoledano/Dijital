...
const limpiarCategoriasVacias = (tienda) => {
    ...
    // Si no hay categorías con productos, mantener al menos 'otros'
    if (categorias.length === 0) {
        categorias = ['otros'];
    }
    
    saveCategorias(tienda, categorias);
    console.log(`📋 Categorías actualizadas para ${tienda}:`, categorias);
    return categorias;
};

// ===========================================
// HELPERS - PRODUCTOS (CORREGIDO)
// ===========================================

const getProductos = (tienda) => {
    const filePath = `./data/productos_${tienda}.json`;
    
    console.log(`🔍 Buscando productos en: ${filePath}`);
    
    if (!fs.existsSync(filePath)) {
        console.log(`📁 Archivo no existe, creando array VACÍO para ${tienda}`);
        const productosVacios = [];
        fs.writeFileSync(filePath, JSON.stringify(productosVacios, null, 2));
        return productosVacios;
    }
    
    try {
        const productos = JSON.parse(fs.readFileSync(filePath));
        console.log(`📦 Cargados ${productos.length} productos para ${tienda}`);
        return productos;
    } catch(e) {
        console.error(`Error leyendo productos de ${tienda}:`, e);
        return [];
    }
};
...
const saveProductos = (tienda, data) => {
    ...
    console.log(`💾 Guardados ${data.length} productos para ${tienda}`);
    // Limpiar categorías vacías después de guardar productos
...
app.get('/api/productos', (req, res) => {
    ...
    const productos = getProductos(tienda);
    console.log(`📤 Enviando ${productos.length} productos para tienda ${tienda}`);
    res.json(productos);
});
